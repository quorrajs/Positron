...
App.prototype.dispatch = function(request, response) {
    this.isDownForMaintenance(function(down){
        ...
        if(down) {
            this.__filter.callFilter('illuminate.app.down', request, response, CB.bind(this));
        } else {
            CB.call(this);
        }
    });
...
App.prototype.down = function(CB)
{
    ...
    this.__filter.register('illuminate.app.down', CB)
};
...
App.prototype.__getStackedClient = function() {
    var client = (new Builder())
        .push(InitializationMiddleware)
        .push(FaviconMiddleware);
