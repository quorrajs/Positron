...
var AuthManager = require('../auth/AuthManager');
...
function start(CB, env) {
     ...
     */
    app.models = (new ModelFactory(app, app.path.app + '/models')).load(function () {
        // expose app globals again so that models also get exposed into the global context
        app.exposeGlobals();

        /**
         * Register auth manager only if auth middleware is enabled.
         *
         * Auth service will be initiated for each request
         * from the auth middleware
         */
        if(app.config.get('middleware').auth) {
            app.__authManager = new AuthManager(app);
        }
    });
