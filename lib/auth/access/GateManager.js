...
GateManager.prototype.define = function (ability, callback) {
    ...
    } else if (_.isString(callback) && callback.indexOf('@') > -1) {
        this.__abilities[ability] = this.__buildAbilityCallback(callback);
    } else {
 ...
 * @return {function}
 */
GateManager.prototype.__buildAbilityCallback = function (callback) {
    return function gateCallback() {
        var parsed = str.parseCallback(callback);
        var policyInstance = new (require(this.__resolvePolicyPath(parsed[0])))();
        var policyMethod = policyInstance[parsed[1]];

        return policyMethod.apply(policyInstance, arguments);
    };
};

/**
 * Resolve policy absolute path.
 *
 * @param policyClass
 * @returns {*}
 * @protected
 */
GateManager.prototype.__resolvePolicyPath = function(policyClass) {
    if(policyClass.indexOf('app/') === 0) {
        policyClass = this.__app.path.base + '/' + policyClass;
    } else if(policyClass.indexOf('policies/') === 0) {
        policyClass = this.__app.path.app + '/' + policyClass;
    }
 ...
 * @return {GateManager}
 */
GateManager.prototype.resource = function (name, policyClass, abilities) {
    abilities = abilities || {
        view: 'view',
        create: 'create',
        update: 'update',
    ...
    _.each(abilities, function (method, ability) {
        this.define(name + '.' +ability, policyClass + '@' + method);
    }, this);
