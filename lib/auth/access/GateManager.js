...
GateManager.prototype.firstArgumentCorrespondsToPolicy = function (methodArguments) {
    ...
    return is_string(methodArguments[0]) && isset(this.__policies[methodArguments[0]]);
};
 ...
 *
 * @param  {Object|String} pType
 * @return {Object}
 *
 * @throws Error
 */
GateManager.prototype.getPolicyFor = function (pType) {
    if (isObject(pType)) {
        pType = pType.pType;
    }

    if (!isset(this.__policies[pType])) {
        throw new Error("Policy not defined for " + pType + ".");
    }
...
GateManager.prototype.executeAfterCallbacks
    ...
    function next() {
        if (afterCallbacks.length) {
            ...
            afterCallback(user, ability, methodArguments, result, next)
