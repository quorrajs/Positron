...
/**
 * Determine if the given ability should be granted for the current user.
 * Error will be thrown in case authorization failure.
 *
 * @param {string} ability
 * @param {array|*} [methodArguments]
 * @param {function} [callback]
 ...
 * @throws Error
 */
Gate.prototype.authorize = function (ability, methodArguments, callback) {
    if (callback === undefined && _.isFunction(methodArguments)) {
        callback = methodArguments;
        methodArguments = [];
    }

    var cb = function (authResponse) {
        if (!authResponse) {
            var error = Error('This action is unauthorized.');
            error.status = 401;
    ...
    this.__raw(ability, methodArguments, cb);
};
...
Gate.prototype.forUser = function (user) {
    return this.__gateManager.getAccessGateForUser(user);
};
...
Gate.prototype.__executeCallbacks = function (user, ability, methodArguments, callback) {
    ...
    self.__gateManager.executeBeforeCallbacks(user, ability, methodArguments, function (result) {
        if (!isset(result)) {
            self.executeAuthorizationCallback(user, ability, methodArguments, function (result) {
 ...
 * @param {Function} callback
 */
Gate.prototype.executeAuthorizationCallback = function (user, ability, methodArguments, callback) {
    var authCallback = this.__resolveAuthorizationCallback(
 ...
 * @protected
 */
Gate.prototype.__resolveAuthorizationCallback = function (user, ability, methodArguments) {
