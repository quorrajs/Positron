...
function compilePattern(route, pattern, isHost)
{
    ...
    while ((match = matchRegex.exec(pattern)) && matches.push(match[0])) {
        ...
        if (null === regexp) {
            ...
            regexp ='[^' + str.regexQuote(defaultSeparator, REGEX_DELIMITER) + (defaultSeparator !== nextSeparator && ''
                !== nextSeparator ? str.regexQuote(nextSeparator, REGEX_DELIMITER) : '') + ']+';
...
function computeRegexp(tokens, index, firstOptional)
{
    ...
    if ('text' === token[0]) {
        // Text tokens
        return str.regexQuote(token[1], REGEX_DELIMITER);
    } else {
        // Variable tokens
        if (0 === index && 0 === firstOptional) {
            // When the only token is an optional variable token, the separator is required
            return str.regexQuote(token[1], REGEX_DELIMITER) + '(?P<'+token[3]+'>'+token[2]+')?';
        } else {
            var regexp = str.regexQuote(token[1], REGEX_DELIMITER) + '(?P<'+ token[3] +'>'+ token[2] +')';
