...
function Router(app, filter){
    ...
    this.bind('_missing', function(value, route, request, CB) { CB(value.split('/')); });
...
{
    return this.__addRoute(methods.map(function(method){return method.toUpperCase();}), uri, action);
};
...
{
    this.bind(key, function(value, route, request, bindExeCallback) {
