...
ControllerInspector.prototype.getRoutable = function(controller, prefix)
{
    ...
    var instance = this.__app.make(path.join(this.__app.path.app, controller), null , true, true);

    //@todo: reflection to get public methods only apart from mocked protected
    ...
    methods.forEach (function(method){
        if (this.isRoutable(method))
        {
            ...
            if (data['plain'] == path.join(prefix, 'index'))
            {
...
{
    //todo: check whether method belongs to controller constructor
...
{
    return path.join(prefix, ((_.snakeCase(name).split('_')).slice(1)).join('-'));
};
...
{
    return path.join(uri,'/{one?}/{two?}/{three?}/{four?}/{five?}');
};
